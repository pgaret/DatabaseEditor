...
The body needs more than a static override: `Staff/<id>/Photo.Body` is a **team-specific** texture the engine has no entry for on these drivers, so a fixed `BODY_OVERRIDES` texture leaves them in unbranded overalls forever, even after signing for a team. Resolve the team kit the same way the engine does for its own `overwriteBodyImageTeam` prop: `StaffConstants/Overalls/F<formula>` `[/<teamID>, F1 only]` `/<0 driver | 1 staff>/<gender>`, read with the staff member's `Photo.BodyType` as the **property name**; team formula comes from `Teams/TeamsList/<teamID>.formula`, and `teamID == -1` means the `Unemployed` set. That lookup alone was not enough: `Photo.BodyType` comes from the same missing `StaffPhotoData` entry, so for these drivers it is empty and the lookup fails. The next fallback copies a teammate's `Photo.Body` (`Teams/TeamsList/<teamID>.driver1ID` / `driver2ID` / `reserveDriverID`, skipping self and other overridden staff), since that is already the team's kit. `BODY_OVERRIDES` is the last resort, and also covers `teamID == -1`. The `overwriteBodyImageTeam` path goes through the same chain for overridden staff, because its engine lookup keys on `BodyType` too. Also listen to `Staff/<id>.teamID`: `Photo.Body` never fires for these drivers, so without it the portrait keeps the old team's kit across a transfer.
