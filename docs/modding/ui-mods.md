...
The body needs more than a static override: `Staff/<id>/Photo.Body` is a **team-specific** texture the engine has no entry for on these drivers, so a fixed `BODY_OVERRIDES` texture leaves them in unbranded overalls forever, even after signing for a team. Resolve the team kit the same way the engine does for its own `overwriteBodyImageTeam` prop: `StaffConstants/Overalls/F<formula>` `[/<teamID>, F1 only]` `/<0 driver | 1 staff>/<gender>`, read with the staff member's `Photo.BodyType` as the **property name**; team formula comes from `Teams/TeamsList/<teamID>.formula`, and `teamID == -1` means the `Unemployed` set. `BODY_OVERRIDES` stays as the fallback when that lookup comes up empty. Also listen to `Staff/<id>.teamID`: `Photo.Body` never fires for these drivers, so without it the portrait keeps the old team's kit across a transfer.
